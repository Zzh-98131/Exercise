	...
	for (function(a) {
			a.fn.qrcode = function(b) {
				...
				return "string" == typeof b && (b = {
					...
					for (a.addData(utf16to8(b.text)), a.make(), c = document.createElement("canvas"), c.width = b.width, c.height =
						b.height, d = c.getContext("2d"), b.src && (e = new Image(), e.src = b.src, e.onload = function() {
							var width = (b.width - b.imgWidth) / 2,
								height = (b.height - b.imgHeight) / 2;
							d.drawImage(e, width, height, b.imgWidth, b.imgHeight)
						}), f = b.width / a.getModuleCount(), g = b.height / a.getModuleCount(), h = 0; h < a.getModuleCount(); h++) {
						for (i = 0; i < a.getModuleCount(); i++) {
							d.fillStyle = a.isDark(h, i) ? b.foreground : b.background, j = Math.ceil((i + 1) * f) - Math.floor(i * f), k =
								Math.ceil((h + 1) * f) - Math.floor(h * f), d.fillRect(Math.round(i * f), Math.round(h * g), j, k)
						}
					}
					return c
				}, d = function() {
					var d, e, f, g, h, i, c = new QRCode(b.typeNumber, b.correctLevel);
					for (c.addData(utf16to8(b.text)), c.make(), d = a("<table></table>").css({
							"width": b.width + "px",
							"height": b.height + "px",
							"border": "0px",
							"border-collapse": "collapse",
							"background-color": b.background
						}), e = b.width / c.getModuleCount(), f = b.height / c.getModuleCount(), g = 0; g < c.getModuleCount(); g++) {
						for (h = a("<tr></tr>").css("height", f + "px").appendTo(d), i = 0; i < c.getModuleCount(); i++) {
							a("<td></td>").css({
								"width": e + "px",
								"background-color": c.isDark(g, i) ? b.foreground : b.background
							}).appendTo(h)
						}
					}
					return d
			...
			makeImpl: function(a, b) {
				...
				this.setupPositionProbePattern(0, 0), this.setupPositionProbePattern(this.moduleCount - 7, 0), this.setupPositionProbePattern(
						0, this.moduleCount - 7), this.setupPositionAdjustPattern(), this.setupTimingPattern(), this.setupTypeInfo(a, b),
					this.typeNumber >= 7 && this.setupTypeNumber(a), null == this.dataCache && (this.dataCache = QRCode.createData(
			...
			setupPositionProbePattern: function(a, b) {
				...
				for (c = -1; 7 >= c; c++) {
					if (!(-1 >= a + c || this.moduleCount <= a + c)) {
						for (d = -1; 7 >= d; d++) {
							-1 >= b + d || this.moduleCount <= b + d || (this.modules[a + c][b + d] = c >= 0 && 6 >= c && (0 == d || 6 ==
								d) || d >= 0 && 6 >= d && (0 == c || 6 == c) || c >= 2 && 4 >= c && d >= 2 && 4 >= d ? !0 : !1)
						}
					}
				}
			},
			...
			createMovieClip: function(a, b, c) {
				...
				for (this.make(), f = 0; f < this.modules.length; f++) {
					for (g = f * e, h = 0; h < this.modules[f].length; h++) {
						i = h * e, j = this.modules[f][h], j && (d.beginFill(0, 100), d.moveTo(i, g), d.lineTo(i + e, g), d.lineTo(i +
							e, g + e), d.lineTo(i, g + e), d.endFill())
					}
				}
				return d
			},
			...
			setupTypeInfo: function(a, b) {
				...
				for (e = 0; 15 > e; e++) {
					f = !a && 1 == (1 & d >> e), 6 > e ? this.modules[e][8] = f : 8 > e ? this.modules[e + 1][8] = f : this.modules[
						this.moduleCount - 15 + e][8] = f
				}
				for (e = 0; 15 > e; e++) {
					f = !a && 1 == (1 & d >> e), 8 > e ? this.modules[8][this.moduleCount - e - 1] = f : 9 > e ? this.modules[8][15 -
						e - 1 + 1
					] = f : this.modules[8][15 - e - 1] = f
				}
			...
			mapData: function(a, b) {
				...
				for (g = this.moduleCount - 1; g > 0; g -= 2) {
					for (6 == g && g--;;) {
						for (h = 0; 2 > h; h++) {
							null == this.modules[d][g - h] && (i = !1, f < a.length && (i = 1 == (1 & a[f] >>> e)), j = QRUtil.getMask(b,
								d, g - h), j && (i = !i), this.modules[d][g - h] = i, e--, -1 == e && (f++, e = 7))
						}
			...
			for (h = 0; h < b.length; h++) {
				for (i = b[h].dataCount, j = b[h].totalCount - i, d = Math.max(d, i), e = Math.max(e, j), f[h] = new Array(i), k =
					0; k < f[h].length; k++) {
					f[h][k] = 255 & a.buffer[k + c]
				}
				for (c += i, l = QRUtil.getErrorCorrectPolynomial(j), m = new QRPolynomial(f[h], l.getLength() - 1), n = m.mod(l),
					g[h] = new Array(l.getLength() - 1), k = 0; k < g[h].length; k++) {
			...
			getLostPoint: function(a) {
				...
				for (d = 0; b - 1 > d; d++) {
					for (e = 0; b - 1 > e; e++) {
						j = 0, a.isDark(d, e) && j++, a.isDark(d + 1, e) && j++, a.isDark(d, e + 1) && j++, a.isDark(d + 1, e + 1) && j++,
							(0 == j || 4 == j) && (c += 3)
					}
				}
				for (d = 0; b > d; d++) {
					for (e = 0; b - 6 > e; e++) {
						a.isDark(d, e) && !a.isDark(d, e + 1) && a.isDark(d, e + 2) && a.isDark(d, e + 3) && a.isDark(d, e + 4) && !a.isDark(
							d, e + 5) && a.isDark(d, e + 6) && (c += 40)
					}
				}
				for (e = 0; b > e; e++) {
					for (d = 0; b - 6 > d; d++) {
						a.isDark(d, e) && !a.isDark(d + 1, e) && a.isDark(d + 2, e) && a.isDark(d + 3, e) && a.isDark(d + 4, e) && !a.isDark(
							d + 5, e) && a.isDark(d + 6, e) && (c += 40)
					}
				}
				for (k = 0, e = 0; b > e; e++) {
					for (d = 0; b > d; d++) {
